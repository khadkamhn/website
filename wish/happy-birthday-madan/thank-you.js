var tq, fr, c = document.getElementById('thank-you'), w = (c.width = window.innerWidth),
...
function anim() {
	tq = window.requestAnimationFrame(anim);
...
// Custom
$('.expo-thanks').on('click',function(){
	var thanks = $('.thank-you').removeClass('d-none');
	if(tq){
		for(var l = 0; l < letters.length; ++l){
			letters[l].reset();
		}
	}
	window.cancelAnimationFrame(tq);
	ctx.clearRect(0, 0, c.width, c.height);
	anim();
	setTimeout(function(){
		thanks.find('.expo').removeClass('d-none').addClass(thanks.find('.expo').data('in'));
	},6500);
});
$('.thank-you .expo-close').on('click',function(){
	var expo = $(this).closest('.thank-you').addClass('d-none').find('.expo');
	expo.each(function(){
		$(this).removeClass($(this).data('in')).removeClass($(this).data('out')).addClass('d-none');
	});
});
$('.expo-restart').on('click',function(){
	//startAgain();
	location.reload();
});
...
function loop() {
	fr = window.requestAnimationFrame(loop);
	hue += 0.5;
...
$('.expo-thanks').on('click',function(){
	window.cancelAnimationFrame(fr);
